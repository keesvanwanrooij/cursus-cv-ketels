...
(function () {
  ...
  /* Via http(s): schone URL's (/les/<slug>/), goed vindbaar voor zoekmachines.
  ...
  if (location.protocol.indexOf('http') === 0) {
    ...
    /* Homepage zonder meta: basis is de map zelf. Offline-fallback op een diepe URL: alles voor /les/, /module/, /naslag/, /kaarten/ of /storingzoeker/. */
    var basis = basisMeta ? basisMeta.content
      : location.pathname.replace(/index\.html$/, '').replace(/^(.*?\/)(?:les|module|naslag|kaarten|storingzoeker)\/.*$/, '$1');
  ...
  function route() {
    ...
    if (r.page === 'module') {
      ...
      markeerNav('naslag');
    } else if (r.page === 'kaarten') {
      app.innerHTML = Views.kaartenPagina();
      markeerNav('kaarten');
    } else if (r.page === 'storingzoeker') {
      app.innerHTML = Views.storingzoekerPagina();
      markeerNav('storingzoeker');
    } else if (r.page === 'casus') {
      app.innerHTML = Views.casusPagina(r.id);
      markeerNav('storingzoeker');
    } else {
  ...
  function koppelLesKnoppen(lesId) {
    ...
    if (knop) {
      knop.addEventListener('click', function () {
        ...
        if (nu) {
          var kaarten = (CURSUS.les(lesId).les.kaarten || []).length;
          melding('Les afgerond.' + (kaarten ? ' ' + kaarten + ' flashcards toegevoegd aan je herhaalschema.' : ' Voortgang opgeslagen.'));
          ...
          if (buren.volgende) {
            setTimeout(function () { ga('les', buren.volgende.les.id); }, 900);
          }
        }
      });
    }
  ...
  function toonMijlpaal(lesId) {
    ...
    /* Alleen als alle lessen (ook de nog geplande) geschreven en afgerond zijn telt het als mijlpaal. */
    var alles = CURSUS.aantallen().lessen === CURSUS.aantallen().lessenKlaar &&
      CURSUS.alleLessen().every(function (o) { return Store.isKlaar(o.les.id); });
    var mv = Store.moduleVoortgang(x.module);
    if (!alles && !(mv.procent === 100 && mv.totaal === mv.alle)) return false;
  ...
  document.querySelectorAll('[data-route="kaarten"]').forEach(function (a) { a.setAttribute('href', Seo.path('kaarten')); });
  document.querySelectorAll('[data-route="storingzoeker"]').forEach(function (a) { a.setAttribute('href', Seo.path('storingzoeker')); });
